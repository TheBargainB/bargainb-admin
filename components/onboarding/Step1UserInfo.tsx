...
const Step1UserInfo: React.FC<Step1UserInfoProps> = (props) => {
  const { name, setName, phone, email, setPhone, setEmail, loading, errors, setErrors, t, setStep, setLoading } = props;
  ...
  return (
    <Card className="w-full max-w-sm sm:max-w-lg mx-auto mt-4 sm:mt-8 shadow-2xl rounded-2xl sm:rounded-3xl border-0 bg-white/80 dark:bg-[#232B23]/80 backdrop-blur-md border border-white/20 dark:border-gray-700/30">
      <CardHeader className="pb-4 sm:pb-6 px-4 sm:px-6">
        ...
        <CardTitle className="text-xl sm:text-2xl md:text-3xl font-bold text-[#1F1F1F] dark:text-[#F5F5F5] text-center mb-2 min-h-[2.5rem] sm:min-h-[3rem] md:min-h-[3.5rem] flex items-center justify-center font-[family-name:var(--font-paytone-one)]">
          {t.welcome}
        </CardTitle>
        <div className="text-sm md:text-base text-[#7A7A7A] dark:text-[#B7EACB] text-center leading-relaxed min-h-[3rem] sm:min-h-[3.5rem] flex items-center justify-center font-[family-name:var(--font-inter)]">
          {t.start}
        </div>
      </CardHeader>
      <CardContent className="space-y-5 sm:space-y-6 px-4 sm:px-6 pb-6 sm:pb-8 font-[family-name:var(--font-inter)]">
        {/* Name Field */}
        <div className="space-y-2">
          <Label htmlFor="name" className="flex items-center gap-2 font-medium text-[#1F1F1F] dark:text-[#F5F5F5] text-sm font-[family-name:var(--font-inter)]">
          ...
          <Input
            ...
            className={`h-11 sm:h-12 text-base transition-all duration-300 focus:scale-[1.02] bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-white/30 dark:border-gray-600/30 font-[family-name:var(--font-inter)] ${errors.name ? 'border-red-500 ring-red-500/20' : 'focus:border-[#00B207] focus:ring-[#00B207]/20'}`}
            aria-invalid={!!errors.name}
            aria-describedby={errors.name ? "name-error" : undefined}
            onKeyDown={(e) => e.key === 'Enter' && handleNext()}
          />
          {errors.name && (
            <div id="name-error" className="text-red-500 text-sm flex items-center gap-1 font-[family-name:var(--font-inter)]">
              <span className="w-1 h-1 bg-red-500 rounded-full"></span>
              {errors.name}
            </div>
          )}
          <div className="text-xs text-[#7A7A7A] dark:text-[#B7EACB] font-[family-name:var(--font-inter)]">
        ...
        {/* Phone Field */}
        <div className="space-y-2">
          <Label htmlFor="phone" className="flex items-center gap-2 font-medium text-[#1F1F1F] dark:text-[#F5F5F5] text-sm font-[family-name:var(--font-inter)]">
          ...
          <div className="relative">
            <Input
              ...
              className={`h-11 sm:h-12 text-base transition-all duration-300 focus:scale-[1.02] bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-white/30 dark:border-gray-600/30 font-[family-name:var(--font-inter)] ${errors.phone ? 'border-red-500 ring-red-500/20' : 'focus:border-[#00B207] focus:ring-[#00B207]/20'}`}
          ...
          {errors.phone && (
            <div id="phone-error" className="text-red-500 text-sm flex items-center gap-1 font-[family-name:var(--font-inter)]">
              <span className="w-1 h-1 bg-red-500 rounded-full"></span>
              {errors.phone}
            </div>
          )}
        ...
        {/* Email Field */}
        <div className="space-y-2">
          <Label htmlFor="email" className="flex items-center gap-2 font-medium text-[#1F1F1F] dark:text-[#F5F5F5] text-sm font-[family-name:var(--font-inter)]">
          ...
          <Input
            ...
            className={`h-11 sm:h-12 text-base transition-all duration-300 focus:scale-[1.02] bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-white/30 dark:border-gray-600/30 font-[family-name:var(--font-inter)] ${errors.email ? 'border-red-500 ring-red-500/20' : 'focus:border-[#00B207] focus:ring-[#00B207]/20'}`}
            aria-invalid={!!errors.email}
            aria-describedby={errors.email ? "email-error" : undefined}
            onKeyDown={(e) => e.key === 'Enter' && handleNext()}
          />
          {errors.email && (
            <div id="email-error" className="text-red-500 text-sm flex items-center gap-1 font-[family-name:var(--font-inter)]">
              <span className="w-1 h-1 bg-red-500 rounded-full"></span>
              {errors.email}
            </div>
          )}
